...
GroupHeadButtonHandler::GroupHeadButtonHandler(GroupButtonMatrixState *groupState, GroupHeadButtonEvent *event, int groupNumber, bool *isInProgrammingMode)
    : event(event),
      isInProgrammingMode(isInProgrammingMode),
      leftSingleDoseButtonHandler(&(groupState->leftSingleDose)),
      leftDoubleDoseButtonHandler(&(groupState->leftDoubleDose)),
      continuousButtonHandler(&groupState->continuous),
      rightSingleDoseButtonHandler(&groupState->rightSingleDose),
      rightDoubleDoseButtonHandler(&groupState->rightDoubleDose),
      groupNumber(groupNumber)
{
}

void GroupHeadButtonHandler::handle()
{
